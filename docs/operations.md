...
- Login sessions expire after 30 days on both the browser and server. Signing out revokes the current session. Sessions are held in server memory, so restarting RemoteX requires signing in again. / 登录会话在浏览器和服务端均于 30 天后过期；退出会撤销当前会话。会话保存在服务端内存中，重启 RemoteX 后需要重新登录。
